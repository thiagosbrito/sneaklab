...
CREATE POLICY "Users can view their order items" ON order_items
  FOR SELECT USING (
    EXISTS (
      ...
      AND (auth.uid()::text = orders.customer_email OR auth.role() = 'authenticated')
    )
  );
...
COMMENT ON TABLE orders IS 'Orders table for custom sneaker business with hand delivery and cash payment';
COMMENT ON TABLE order_items IS 'Individual items in each order with customization details';
